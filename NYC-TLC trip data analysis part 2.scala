...
// ENTER THE CODE BELOW
df_filter.groupBy("DOLocationID").count().withColumnRenamed("count", "number_of_dropoffs").orderBy(desc("number_of_dropoffs"), asc("DOLocationID")).limit(5)
...
// ENTER THE CODE BELOW
df_filter.groupBy("PULocationID").count().withColumnRenamed("count", "number_of_pickups").orderBy(desc("count"), asc("PULocationID")).limit(5)
...
// ENTER THE CODE BELOW
val join_df = zone_df.join(df_filter, $"LocationID"===$"DOLocationID" || $"LocationID"===$"PULocationID")
join_df.groupBy("LocationID").count().withColumnRenamed("count", "number_activities").orderBy(desc("number_activities"), asc("LocationID")).limit(3)
...
// ENTER THE CODE BELOW
join_df.groupBy("Borough").count().withColumnRenamed("count", "total_number_activities").orderBy(desc("total_number_activities"))
...
// ENTER THE CODE BELOW
val df1 = df_filter.withColumn("date", to_date(col("pickup_datetime"), "yyyy-MM-dd")).groupBy("date").count()
df1.withColumn("day_of_week", date_format(col("date"), "EEEE")).groupBy("day_of_week").agg(avg("count").alias("avg_count")).orderBy(desc("avg_count")).limit(2)
...
// ENTER THE CODE BELOW
val df_hour = zone_df.join(df_filter, $"LocationID"===$"PULocationID").filter($"Borough" === "Brooklyn").withColumn("hour_of_day", hour(col("pickup_datetime"))).groupBy("hour_of_day", "zone").count()
val window = Window.partitionBy("hour_of_day")
df_hour.withColumn("max_count", max("count").over(window)).filter($"count"===$"max_count").drop("count").orderBy(asc("hour_of_day"))
...
// ENTER THE CODE BELOW
val df6 = zone_df.join(df_filter, $"LocationID"===$"PULocationID").filter($"Borough" === "Manhattan")
          .withColumn("day", dayofmonth(col("pickup_datetime"))).filter(month(col("pickup_datetime")) === 1).groupBy("day").count()
          .withColumn("prev_count", lag(col("count"), 1, 0).over(Window.orderBy("day")))
          .withColumn("percent_change", round((col("count") - col("prev_count")) / col("prev_count")  * 100 , 2)).select("day", "percent_change").orderBy(desc("percent_change")).limit(3)
